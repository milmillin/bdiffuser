...
export function awardMission54ValidationOxygenBonus(state: GameState): void {
  if (state.mission !== 54 && state.mission !== 63) return;
...
function usesAnnouncedFalseTokenForDualCut(
  ...
  return (
    falseTokenMode
    || (falseInfoTokenMode && (actor.isCaptain || target.isCaptain))
  );
}
...
export function executeDualCut(
  ...
  if (dualCutSuccess) {
    ...
    if (!suppressInfoTokens) {
      const usesAnnouncedFalseToken = usesAnnouncedFalseTokenForDualCut(
        state,
        actor,
        target,
      );
  ...
  if (!allMatch) {
    ...
    if (requiredColor === "yellow") {
      ...
      addLog(
        ...
        actorId,
        "simultaneousRedCut",
        `designated 3 wires for ${actionLabel} — mismatch. Info tokens placed and detonator advanced by 1`,
      );
...
function resolveDoubleDetectorNoMatch(
  ...
  const suppressInfoTokens = failureInfoTokenMode === "none";
  if (!suppressInfoTokens) {
    const usesAnnouncedFalseToken = usesAnnouncedFalseTokenForDualCut(state, actor, target);
    const tokenValue =
      usesAnnouncedFalseToken
        ? guessValue
